...
class LabelingWidget(LabelDialog):
    ...
    def save_attributes(self, _shapes):
        ...
        # Get current shapes
        # Excluding auto labeling special shapes and band polygons
        shapes = [
            format_shape(shape)
        ...
        # If in band mode, also save centerlines so track_width persists
        try:
            aw = self.auto_labeling_widget
            if aw._band_mode and hasattr(aw, "_centerline_shapes"):
                for cl_s in aw._centerline_shapes:
                    if cl_s.label not in [
                        AutoLabelingMode.OBJECT,
                        AutoLabelingMode.ADD,
                        AutoLabelingMode.REMOVE,
                    ]:
                        shapes.append(format_shape(cl_s))
        except Exception:
            pass
        flags = {}
    ...
    def load_file(self, filename=None):  # noqa: C901
        ...
        try:
            self.auto_labeling_widget.reset_band_state()
        except Exception:
            pass
        self.reset_state()
