...
class Canvas(
    ...
    def can_erase_selected_vertices(self):
        if not self.editing() or len(self.selected_shapes) != 1:
            return False
        if self.selected_shapes[0].other_data.get("_is_band"):
            return False
    ...
    def mouseMoveEvent(self, ev):  # noqa: C901
        ...
        for shape in reversed([s for s in self.shapes if self.is_visible(s)]):
            ...
            is_band = shape.other_data.get("_is_band", False)
            index = shape.nearest_vertex(pos, self.epsilon / self.scale) if not is_band else None
            index_edge = shape.nearest_edge(pos, self.epsilon / self.scale) if not is_band else None
            if index is not None:
                if self.selected_vertex():
                    self.h_shape.highlight_clear()
            ...
            if shape.shape_type in ["point", "line", "linestrip"]:
                ...
                if nearest_index is not None and not shape.other_data.get("_is_band"):
                    shape_hit = True
            ...
            if shape_hit:
                ...
                self.h_cuboid_face = None
                if is_band:
                    self.setToolTip(self.tr("Click to select '%s'") % shape.label)
                    self.override_cursor(CURSOR_DEFAULT)
        ...
        if ev.button() == QtCore.Qt.MouseButton.LeftButton:
            ...
            if self.drawing():
                ...
                if (
                    ...
                    and ev.modifiers()
                    != QtCore.Qt.KeyboardModifier.ShiftModifier
                    and not (self.h_shape and self.h_shape.other_data.get("_is_band"))
                ):
                ...
                self.prev_pan_point = ev.position()
                self.repaint()
                # If any selected shape is a band polygon and click was on blank, deselect all
                any_band = any(s.other_data.get("_is_band") for s in self.selected_shapes)
                if any_band and self.h_shape is None:
            ...
            for shape in reversed(self.shapes):
                ...
                if shape.shape_type in ["point", "line", "linestrip"]:
                    if (
                        ...
                        is not None
                        and not shape.other_data.get("_is_band")
                    ):
                        hit = True
                elif len(shape.points) > 1 and shape.contains_point(pos):
                    hit = True
    ...
    def select_shape_point(self, point, multiple_selection_mode):
        ...
        if self.selected_vertex():  # A vertex is marked for selection.
            ...
            # Band polygons: select only, no vertex editing
