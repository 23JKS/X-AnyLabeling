...
class AutoLabelingWidget(QWidget):
    ...
    def __init__(self, parent):
        ...
        # Track centerline widgets
        self.edit_min_track_length.valueChanged.connect(
            self.on_min_track_length_changed
        )
        self.edit_keypoint_interval.valueChanged.connect(
            self.on_keypoint_interval_changed
        )
        self.edit_track_width.valueChanged.connect(
            self.on_track_width_changed
        )
        self.button_toggle_band.clicked.connect(
            self._on_toggle_band
        )
        self.edit_per_track_width.valueChanged.connect(
            self.on_per_track_width_changed
        )
    ...
    def on_min_track_length_changed(self, value):
        """Handle minimum track length slider change."""
        self.model_manager.set_auto_labeling_min_track_length(value)

    def on_keypoint_interval_changed(self, value):
        """Handle keypoint sampling interval slider change."""
        self.model_manager.set_auto_labeling_keypoint_interval(value)

    def on_per_track_width_changed(self, value):
        """Handle per-track width change for selected polygon."""
        ...
        if cl:
            ...
            for cl_s in self._centerline_shapes:
                if cl_s.label == label:
                    cl_s.other_data["track_width"] = value
                    break
            else:
                for s in self.parent.canvas.shapes:
                    if s.label == label and s.shape_type in ("linestrip", "line") and not s.other_data.get("_is_band"):
                        s.other_data["track_width"] = value
                        break
        self.parent.canvas.update()

    def on_track_width_changed(self, value):
        """Handle track width slider change."""
        self.model_manager.set_auto_labeling_track_width(value)
        if self._editing_track_shape is None:
            self._default_track_width = value
            return
        cl = self._editing_track_shape.other_data.get("centerline")
        if cl:
            new_pts = self._regenerate_band_polygon(cl, value / 2.0)
    ...
    def _regenerate_band_polygon(self, points, half_width):
    ...
    def _on_toggle_band(self):
        """Toggle band polygon expansion on/off."""
        ...
        if self._band_mode:
            ...
            self.edit_track_width.setValue(self._default_track_width)
            self.input_track_width.setText(self.tr("新扩展多边形的默认宽度"))
            self.input_per_track_width.hide()
            self.edit_per_track_width.hide()
        self.button_toggle_band.setText(self.tr("隐藏带状") if self._band_mode else self.tr("扩展为带状"))
        self.parent.canvas.update()
    ...
    def _on_shape_selected(self, selected_shapes):
        ...
        if len(selected_shapes) == 1 and self._band_mode:
            ...
            if shape.other_data.get("_is_band") and shape.other_data.get("track_width"):
                ...
                self.edit_per_track_width.blockSignals(True)
                self.edit_per_track_width.setValue(int(shape.other_data["track_width"]))
                self.edit_per_track_width.blockSignals(False)
                self.input_per_track_width.setText(self.tr("{label} 当前宽度").format(label=shape.label))
                self.input_per_track_width.show()
                self.edit_per_track_width.show()
                return
        # Deselected: hide per-track width controls
        self.input_per_track_width.hide()
        self.edit_per_track_width.hide()

    def reset_band_state(self):
        """Reset band mode when switching images."""
        if self._band_mode:
            self.parent.canvas.shapes = [s for s in self.parent.canvas.shapes if not s.other_data.get("_is_band")]
            existing = {id(s): s for s in self.parent.canvas.shapes}
            for cl_s in self._centerline_shapes:
        ...
        self._band_shapes = []
        self._editing_track_shape = None
        self.button_toggle_band.setText(self.tr("扩展为带状"))
        self.input_track_width.show()
        self.edit_track_width.show()
        self.edit_track_width.setValue(self._default_track_width)
        self.input_track_width.setText(self.tr("新扩展多边形的默认宽度"))
